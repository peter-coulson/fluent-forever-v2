...
class CardType(ABC):
    ...
    @abstractmethod
    def find_card_by_id(self, data: Dict[str, Any], card_id: str) -> Optional[Dict[str, str]]:
...
class ConjugationCardType(CardType):
    ...
    def build_fields(self, card_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        if 'Picture' in fields and fields['Picture']:
            ...
            if picture and '/' not in picture and not picture.startswith('media/images/'):
...
class CardTypeRegistry:
    ...
    def get(self, name: str) -> Optional[CardType]:
