...
def get_pipeline_media_paths(pipeline_name: str, project_root: Path = None) -> Dict[str, Path]:
...
def get_pipeline_template_paths(pipeline_name: str, anki_note_type: str, project_root: Path = None) -> Dict[str, Path]:
...
def get_pipeline_data_paths(pipeline_name: str, data_file: str, project_root: Path = None) -> Dict[str, Path]:
...
def ensure_pipeline_directories(pipeline_name: str, anki_note_type: str, data_file: str, project_root: Path = None) -> None:
    ...
    template_paths = get_pipeline_template_paths(pipeline_name, anki_note_type, project_root)
...
def validate_pipeline_isolation(pipeline_name: str, anki_note_type: str) -> Dict[str, Any]:
    ...
    if pipeline_name != 'vocabulary':
        ...
        if media_paths['base'] == project_root / "media":
            issues.append(f"Pipeline '{pipeline_name}' not properly isolated - using root media directory")
    ...
    for other_pipeline in ['vocabulary', 'conjugation']:
        ...
        other_note_type = 'Fluent Forever' if other_pipeline == 'vocabulary' else 'Conjugation'
        other_paths = get_pipeline_template_paths(other_pipeline, other_note_type, project_root)
        
        if template_paths['base'] == other_paths['base']:
            issues.append(f"Template path conflict between '{pipeline_name}' and '{other_pipeline}'")
    
    # Check data file isolation
    data_paths = get_pipeline_data_paths(pipeline_name, f"{pipeline_name}s.json" if pipeline_name != 'vocabulary' else 'vocabulary.json', project_root)
