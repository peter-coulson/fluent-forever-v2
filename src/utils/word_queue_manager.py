...
class WordQueueManager:
    ...
    def load_word_queue(self) -> Dict[str, Any]:
        ...
        if not self.word_queue_path.exists():
            self.logger.warning(f"{ICONS['warning']} Word queue file not found: {self.word_queue_path}")
            return {
                "metadata": {
    ...
    def validate_words_in_queue(self, words_to_check: List[str]) -> tuple[List[str], List[str]]:
        """
        ...
        words_not_in_queue = [word for word in words_to_check if word not in queue_words_set]
        
        return words_in_queue, words_not_in_queue

    def remove_words_from_queue(self, words_to_remove: List[str], reason: str = "processed", require_exists: bool = False) -> Dict[str, Any]:
        """
        ...
        if require_exists:
            words_in_queue, words_not_in_queue = self.validate_words_in_queue(words_to_remove)
            if words_not_in_queue:
                raise ValueError(f"Words not found in queue: {', '.join(words_not_in_queue)}")
        ...
        updated_words = [word for word in original_words if word not in words_to_remove_set]
        ...
        if removed_count > 0:
            ...
            metadata[f'removed_{reason}'] = metadata.get(f'removed_{reason}', 0) + removed_count
            queue['metadata'] = metadata
            
            self.logger.info(f"{ICONS['check']} Removed {removed_count} {reason} words from queue")
        else:
            self.logger.info(f"{ICONS['info']} No {reason} words found in queue to remove")
        
        return queue

    def update_queue_after_batch_processing(self, processed_words: List[str], skipped_words: List[str] = None) -> None:
        """
        ...
        updated_queue = self.remove_words_from_queue(all_words_to_remove, "batch_processing")
        ...
        self.logger.info(f"{ICONS['check']} Queue updated: removed {total_removed} words "
    ...
    def sync_queue_with_vocabulary(self) -> None:
        ...
        if not all_words_to_remove:
            self.logger.info(f"{ICONS['info']} Queue is already synchronized with vocabulary")
            return
        
        updated_queue = self.remove_words_from_queue(all_words_to_remove, "synchronization")
        self.save_word_queue(updated_queue)
        
        self.logger.info(f"{ICONS['check']} Queue synchronized: removed {len(processed_words)} processed "
    ...
    def save_word_queue(self, queue: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(f"{ICONS['check']} Word queue saved: {self.word_queue_path}")
        except Exception as e:
