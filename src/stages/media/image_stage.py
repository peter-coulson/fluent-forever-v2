...
class ImageGenerationStage(APIStage):
        ...
        try:
            ...
            if len(plan.images_to_generate) > self.max_new:
                return StageResult(
                    ...
                    data={},
                    errors=[f"Exceeded max images limit: {len(plan.images_to_generate)} > {self.max_new}"]
                )
    ...
    def load_cards_from_vocabulary(self, project_root: Path, card_ids: List[str]) -> List[Dict[str, Any]]:
    ...
    def generate_images(self, project_root: Path, plan, provider):
        ...
        for image_file in sorted(plan.images_to_generate):
            ...
            if not meaning:
                self.logger.warning(f"{ICONS['warning']} No meaning found for image {image_file}")
                skipped += 1
                continue
            ...
            if existing_prov and existing_prov.get('prompt_hash') != prompt_hash and not self.force_regenerate:
                self.logger.warning(f"{ICONS['warning']} Prompt changed for {image_file}; keeping existing. Use --force-regenerate to update.")
                skipped += 1
                continue
