        ...
        try:
            ...
            if len(plan.audio_to_generate) > self.max_new:
                return StageResult(
                    ...
                    data={},
                    errors=[f"Exceeded max audio limit: {len(plan.audio_to_generate)} > {self.max_new}"]
                )
            ...
            context.set('audio_generation_stats', {
            ...
            message = f"Generated {results.generated} audio files, skipped {results.skipped}"
            if results.failed > 0:
