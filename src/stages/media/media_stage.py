...
class MediaGenerationStage(Stage):
    ...
    def __init__(self, max_new: int = 5, skip_images: bool = False, skip_audio: bool = False, 
        ...
        # Create sub-stages
        if not skip_images:
            self.image_stage = ImageGenerationStage(max_new=max_new, force_regenerate=force_regenerate)
        if not skip_audio:
    ...
    def execute(self, context: PipelineContext) -> StageResult:
        ...
        image_ok = self.skip_images or (results['image_result'] and results['image_result'].status != StageStatus.FAILURE)
        audio_ok = self.skip_audio or (results['audio_result'] and results['audio_result'].status != StageStatus.FAILURE)
