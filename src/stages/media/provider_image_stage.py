...
from providers.base.media_provider import MediaRequest, MediaProvider
...
class ProviderImageGenerationStage(APIStage):
    ...
    def execute_api_call(self, context: PipelineContext, provider: MediaProvider) -> StageResult:
        ...
        try:
            ...
            if not provider.supports_type('image'):
                return StageResult(
                    ...
                    message=f"Provider does not support image generation",
                    data={},
                    errors=[f"Provider {self.provider_key} does not support 'image' type"]
                )
            ...
            for i, request_data in enumerate(requests):
                try:
                    result = self._generate_single_image(provider, request_data, context)
                    if result['success']:
                        generated_images.append(result)
                        self.logger.info(f"{ICONS['check']} Generated image {i+1}/{len(requests)}")
                    else:
                        errors.append(f"Image {i+1}: {result.get('error', 'Unknown error')}")
            ...
            if success_count == total_count:
                ...
                status = StageStatus.FAILURE
                message = f"Failed to generate any images"
    ...
    def _generate_single_image(self, provider: MediaProvider, request_data: Dict[str, Any], context: PipelineContext) -> Dict[str, Any]:
