    ...
    def execute(self, context: PipelineContext) -> StageResult:
        ...
        try:
            ...
            if already_processed:
                self.logger.info(f"{ICONS['info']} Skipping {len(already_processed)} already processed words: {already_processed}")
            if already_skipped:
                self.logger.info(f"{ICONS['info']} Skipping {len(already_skipped)} previously skipped words: {already_skipped}")
            
            if not new_words:
                self.logger.warning(f"{ICONS['warning']} All words have already been processed or skipped")
                return StageResult(
            ...
            self.logger.info(f"{ICONS['check']} Analyzed {len(new_words)} words with {len(analyzed_meanings)} total meanings")
