...
class BatchIngestionStage(Stage):
    ...
    def __init__(self, skip_ipa_validation: bool = False, update_word_queue: bool = True):
        """
        ...
        try:
            ...
            for meaning in meanings:
                ...
                required = [
                    ...
                    'ExampleSentence', 'GappedSentence', 'IPA', 'prompt'
                ]
                ...
                if '_____' not in str(meaning['GappedSentence']):
                    validation_errors.append(f"GappedSentence must contain '_____': {meaning['GappedSentence']}")
                    continue
            ...
            if self.update_word_queue:
                queue_result = self.update_word_queue_file(project_root, derived, skipped_words)
                if not queue_result.success:
                    self.logger.warning(f"{ICONS['warning']} Word queue update failed: {queue_result.message}")
            ...
            for word, meanings_list in derived.items():
                ...
                if ipa_raw:
                    try:
                        ...
                        self.logger.warning(f"{ICONS['warning']} IPA validation error for '{word}': {e}")
            
            if ipa_failures:
                self.logger.error(f"{ICONS['cross']} IPA validation failed for {len(ipa_failures)} words:")
                for failure in ipa_failures:
                    self.logger.error(f"  - {failure['word']} → {failure['predicted_ipa']} ({failure['meanings_count']} meanings)")
                
                return StageResult(
                    status=StageStatus.FAILURE,
                    message=f"IPA validation failed for {len(ipa_failures)} words",
                    data={},
                    errors=[f"IPA validation failed for: {[f['word'] for f in ipa_failures]}"]
                )
            
            self.logger.info(f"{ICONS['check']} All IPA pronunciations validated successfully")
            return StageResult(status=StageStatus.SUCCESS, message="IPA validation passed", data={}, errors=[])
            
        except ImportError:
            self.logger.warning(f"{ICONS['warning']} IPA validator not available, skipping validation")
            return StageResult(status=StageStatus.SUCCESS, message="IPA validation skipped", data={}, errors=[])
    ...
    def process_skipped_words(self, vocab: Dict[str, Any], skipped_words: List[str]) -> int:
    ...
    def merge_meanings(self, vocab: Dict[str, Any], derived: Dict[str, List[Dict[str, Any]]]) -> int:
    ...
    def validate_vocabulary_schema(self, vocab: Dict[str, Any], project_root: Path) -> StageResult:
        ...
        try:
            ...
            return StageResult(status=StageStatus.SUCCESS, message="Schema validation passed", data={}, errors=[])
            
        except ImportError:
            self.logger.warning(f"{ICONS['warning']} Vocabulary validator not available, skipping schema validation")
            return StageResult(status=StageStatus.SUCCESS, message="Schema validation skipped", data={}, errors=[])
    
    def update_word_queue_file(self, project_root: Path, derived: Dict[str, List[Dict[str, Any]]], skipped_words: List[str]) -> StageResult:
        ...
        try:
            ...
            return StageResult(status=StageStatus.SUCCESS, message="Word queue updated", data={}, errors=[])
