...
class CardSyncStage(APIStage):
    ...
    def execute_api_call(self, context: PipelineContext, provider: Any) -> StageResult:
        ...
        try:
            ...
            self.logger.info(f"{ICONS['check']} Cards synced: created {card_summary['created']}, "
            ...
            if not validation_result.is_synchronized:
                ...
                return StageResult(
                    ...
                    errors=[f"Sync validation found {len(validation_result.get_differences())} differences"]
                )
            ...
            return StageResult(
                ...
                message=f"Cards synchronized successfully",
    ...
    def upsert_cards(self, provider, vocabulary: Dict[str, Any]) -> Dict[str, int]:
        ...
        try:
            ...
            self.logger.warning(f"{ICONS['warning']} Card upsert not available (sync module missing)")
    ...
    def handle_extra_cards(self, provider, vocabulary: Dict[str, Any]):
        ...
        try:
            ...
            if extras:
                ...
                self.logger.warning(f"{ICONS['warning']} Found {len(extras)} extra notes in deck '{deck}'")
                
                # For stage execution, we don't do interactive confirmation
                # Instead, we log the extras and let the user handle them manually
                self.logger.warning(f"{ICONS['info']} Extra cards found but not deleted (requires manual confirmation)")
            ...
            self.logger.warning(f"{ICONS['warning']} Extra card handling not available (sync module missing)")
            return None
    ...
    def log_sync_differences(self, validation_result):
        ...
        try:
            ...
            if hasattr(validation_result, 'missing_words_in_anki') and validation_result.missing_words_in_anki:
                self.logger.warning(f"{ICONS['warning']} Missing words in Anki: {sorted(validation_result.missing_words_in_anki)}")
            
            if hasattr(validation_result, 'missing_words_in_vocab') and validation_result.missing_words_in_vocab:
                self.logger.warning(f"{ICONS['warning']} Words in Anki but missing in vocabulary: {sorted(validation_result.missing_words_in_vocab)}")
            
            if hasattr(validation_result, 'field_differences') and validation_result.field_differences:
                self.logger.warning(f"{ICONS['warning']} Field differences found (showing first 5):")
                for i, diff in enumerate(validation_result.field_differences[:5]):
                    self.logger.warning(f"   {diff.word}.{diff.meaning_id} → {diff.field}: vocab='{diff.vocab_value}' vs anki='{diff.anki_value}'")
                if len(validation_result.field_differences) > 5:
                    self.logger.warning(f"   (+{len(validation_result.field_differences) - 5} more differences)")
                    
        except Exception as e:
            self.logger.warning(f"{ICONS['warning']} Could not log sync differences: {e}")