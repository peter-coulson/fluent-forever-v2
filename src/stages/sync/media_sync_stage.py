...
class MediaSyncStage(APIStage):
    ...
    def execute_api_call(self, context: PipelineContext, provider: Any) -> StageResult:
        ...
        try:
            ...
            if self.force_media:
                ...
                self.logger.info(f"{ICONS['info']} Force mode: uploading all {len(ref_images)} images and {len(ref_audio)} audio files")
            else:
                ...
                self.logger.info(f"{ICONS['info']} Found {len(missing_images)} missing images and {len(missing_audio)} missing audio files")
            
            # Upload missing media
            upload_result = self.upload_missing_media(provider, project_root, missing_images, missing_audio)
    ...
    def list_anki_media(self, provider) -> tuple[Set[str], Set[str]]:
        ...
        try:
            ...
            self.logger.warning(f"{ICONS['warning']} Cannot list Anki media (sync module missing)")
            return set(), set()
    
    def compute_missing_media(self, ref_images: Set[str], ref_audio: Set[str], 
                            anki_images: Set[str], anki_audio: Set[str]) -> tuple[Set[str], Set[str]]:
    ...
    def upload_missing_media(self, provider, project_root: Path, missing_images: Set[str], missing_audio: Set[str]):
        ...
        try:
            ...
            if total_files > 0:
                try:
                    push_missing_media(provider, project_root, missing_images, missing_audio)
                    uploaded = total_files  # Assume all succeeded if no exception
                    self.logger.info(f"{ICONS['check']} Uploaded {total_files} media files")
                except Exception as e:
                    failed = total_files
            ...
            self.logger.warning(f"{ICONS['warning']} Media upload not available (sync module missing)")
            return {
                'uploaded': 0,
                'failed': 0,
