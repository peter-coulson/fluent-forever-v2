...
class DataValidationStage(ValidationStage):
    ...
    def validate_vocabulary_structure(self, vocab: Dict[str, Any]) -> List[str]:
        ...
        if 'metadata' in vocab:
            ...
            if not isinstance(metadata, dict):
                ...
                required_meta = ['created', 'last_updated', 'total_words', 'total_cards']
    ...
    def validate_meaning_entry(self, word: str, index: int, meaning: Dict[str, Any]) -> List[str]:
        ...
        required_fields = [
            ...
            'ExampleSentence', 'GappedSentence', 'IPA', 'CardID', 'ImageFile'
        ]
        
        for field in required_fields:
            if field not in meaning or not meaning[field]:
                errors.append(f"Word '{word}' meaning {index} missing required field: {field}")
        
        # Validate specific field formats
        if 'GappedSentence' in meaning:
            gapped = meaning['GappedSentence']
            if '_____' not in str(gapped):
                errors.append(f"Word '{word}' meaning {index} GappedSentence must contain '_____'")
        ...
        if 'CardID' in meaning and 'SpanishWord' in meaning and 'MeaningID' in meaning:
            ...
            if meaning['CardID'] != expected_id:
                errors.append(f"Word '{word}' meaning {index} CardID mismatch: expected '{expected_id}', got '{meaning['CardID']}'")
