...
class BaseAPIClient(ABC):
    ...
    def load_config(cls, config_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        if cls._shared_config is None:
            ...
            try:
                ...
                logger.error(f"{ICONS['cross']} Failed to load config from {config_path}: {e}")
                raise
    ...
    def __init__(self, service_name: str):
        ...
        self.logger = get_logger(f'apis.{service_name.lower()}')
    ...
    def _setup_session(self):
        ...
        self.session.timeout = base_config["timeout"]
    ...
    def _make_request(
        ...
        **kwargs
        ...
        if max_retries is None:
            max_retries = self.config["apis"]["base"]["max_retries"]
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 429:
                    ...
                    self.logger.warning(f"{ICONS['warning']} Rate limited by {self.service_name}. Waiting {retry_after}s...")
                ...
                if response.status_code < 400:
                    try:
                        ...
                        return APIResponse(success=True, data=response.content, status_code=response.status_code)
                
                # Handle client/server errors
                error_msg = f"{self.service_name} API error (HTTP {response.status_code})"
                try:
                ...
                self.logger.warning(f"{ICONS['warning']} Server error (HTTP {response.status_code}), retrying...")
        ...
        final_error = f"Failed to connect to {self.service_name} after {max_retries} attempts"
        if last_exception:
...
# Config loading is now handled by BaseAPIClient.load_config()

def ensure_media_directories(config: Dict[str, Any]) -> None:
    """Ensure media directories exist"""
    media_folder = Path(config["paths"]["media_folder"])
