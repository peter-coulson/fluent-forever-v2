...
class AnkiProvider(SyncProvider, BaseAPIClient):
    ...
    def __init__(self):
        ...
        elif "providers" in self.config and "sync" in self.config["providers"] and "anki" in self.config["providers"]["sync"]:
        ...
        try:
            ...
            if response.success and response.data:
                ...
                if version and version >= 6:
                    ...
                    self.logger.warning(f"{ICONS['warning']} AnkiConnect version too old: {version}")
    ...
    def _sync_cards(self, cards: List[Dict[str, Any]], params: Dict[str, Any]) -> SyncResult:
        ...
        return SyncResult(
            ...
            metadata={'operation': 'sync_cards', 'deck': deck_name, 'note_type': note_type},
            created_ids=[nid for nid in note_ids if nid is not None]
        )
    
    def _sync_templates(self, template_data: Dict[str, Any], params: Dict[str, Any]) -> SyncResult:
    ...
    def _sync_media(self, media_files: List[Dict[str, Any]], params: Dict[str, Any]) -> SyncResult:
        ...
        for media_file in media_files:
            ...
            try:
                ...
                if response.success:
                    ...
                    logger.warning(f"{ICONS['warning']} Failed to store media file {filename}: {response.error_message}")
                    
            except Exception as e:
                logger.error(f"{ICONS['cross']} Error processing media file {filename}: {e}")
