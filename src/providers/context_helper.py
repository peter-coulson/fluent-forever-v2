...
def setup_providers_in_context(
    context: PipelineContext, config: dict | None = None
) -> None:
...
def _setup_data_providers(
    context: PipelineContext, factory: DataProviderFactory, config: dict
) -> None:
...
def _setup_media_providers(
    context: PipelineContext, factory: MediaProviderFactory, config: dict
) -> None:
    ...
    try:
        ...
        if primary_media_provider:
            ...
            logger.warning(
                f"{ICONS['warning']} Failed to create primary media provider: {primary_provider}"
            )
    except Exception as e:
...
def _setup_fallback_providers(
    context: PipelineContext, factory: MediaProviderFactory, primary: str
) -> None:
...
def _setup_sync_providers(
    context: PipelineContext, factory: SyncProviderFactory, config: dict
) -> None:
...
def get_provider_from_context(
    context: PipelineContext, provider_type: str, provider_name: str | None = None
) -> Any:
...
def register_provider_in_context(
    context: PipelineContext, provider_key: str, provider: Any
) -> None:
