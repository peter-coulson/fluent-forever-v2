...
class TestCreateParser:
    ...
    def test_run_command(self):
        ...
        args = parser.parse_args(
            ["run", "vocabulary", "--stage", "prepare", "--words", "test1,test2"]
        )
        assert args.command == "run"
        ...
        # Run with dry-run
        args = parser.parse_args(
            ["run", "vocabulary", "--stage", "prepare", "--dry-run"]
        )
        assert args.command == "run"
        assert args.dry_run
        ...
        args = parser.parse_args(
            ["run", "vocabulary", "--stage", "media", "--cards", "card1", "--execute"]
        )
        assert args.command == "run"
    ...
    def test_preview_command(self):
        ...
        args = parser.parse_args(
            ["preview", "vocabulary", "--start-server", "--port", "8001"]
        )
        assert args.command == "preview"
