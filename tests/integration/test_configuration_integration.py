...
def test_config_loading_integration():
    ...
    test_config = {
        ...
        "paths": {
            "media_base": "test_media",
            "templates_base": "test_templates"
        },
        "providers": {
            "openai": {"enabled": True},
            "runware": {"enabled": False}
        }
    }
    ...
    try:
        ...
        if hasattr(config_manager, 'load_config_from_file'):
            ...
            assert loaded_config.get("system", {}).get("log_level") == "DEBUG", "Should load correct values"
...
def test_environment_variable_integration():
    ...
    assert resolve_environment_variables is not None, "resolve_environment_variables should be available"
