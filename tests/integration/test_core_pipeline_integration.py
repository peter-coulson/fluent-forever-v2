...
def test_pipeline_registry_methods():
    ...
    ), "Registry should have list_pipelines method"
    ...
    if pipelines and len(pipelines) > 0:
        first_pipeline = (
            list(pipelines)[0] if isinstance(pipelines, dict) else pipelines[0]
        )
...
def test_pipeline_context_integration():
    ...
    context = PipelineContext(
        pipeline_name="test", project_root=Path("."), data={"test": "data"}
    )
