...
from tests.fixtures.configs import (
    ...
    create_no_providers_config,
)
from tests.utils.assertions import (
    assert_config_loaded_correctly,
    assert_environment_substitution,
)
    ...
    def test_nested_environment_substitution(self):
        ...
        config_data = {
            "providers": {
                "data": {
                    "nested_provider": {
                        "type": "json",
                        "pipelines": ["${PIPELINE_1:default1}", "${PIPELINE_2:default2}"],
                        "config": {
                            "nested_setting": "${NESTED_VAR:nested_default}",
                            "deep": {
                                "value": "${DEEP_VAR:deep_default}"
                            }
                        }
                    }
                }
        ...
        with pytest.MonkeyPatch().context() as mp:
            ...
            with ConfigFixture(config_data) as config_path:
                ...
                assert provider_config["config"]["nested_setting"] == "custom_nested_value"
    ...
    def test_recursive_environment_substitution(self):
        ...
        config_data = {
            ...
            "dict_with_env": {
                ...
                "key2": {
                    "nested_key": "${DICT_VAR2:dict_value2}"
                }
            }
        }
