...
class TestProviderRegistryIntegration:
    ...
    def test_provider_registry_from_config(self, complex_config_file):
        ...
        assert_providers_registered(
            registry, "data", ["vocabulary_data", "conjugation_data", "shared_data"]
        )
        assert_providers_registered(
            registry, "audio", ["primary_audio", "backup_audio"]
        )
        assert_providers_registered(
            registry, "image", ["openai_images", "runware_images"]
        )
    ...
    def test_provider_pipeline_isolation(self, complex_config_file):
        ...
        new_pipeline_providers = registry.get_providers_for_pipeline(
        ...
        assert (
            "vocabulary_data" not in new_pipeline_providers["data"]
        )  # Specific to vocabulary
        assert (
            "conjugation_data" not in new_pipeline_providers["data"]
        )  # Specific to conjugation
