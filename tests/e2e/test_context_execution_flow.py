...
from tests.fixtures.pipelines import MultiPhasePipeline, MockPipeline
...
class TestContextExecutionFlow:
    ...
    def test_cli_run_single_stage_dry_run(self, config_file, pipeline_registry, provider_registry, capsys):
        """Test CLI run command with single stage dry run."""
        test_args = ["--config", str(config_file), "run", "test_pipeline", "--stage", "test_stage", "--dry-run"]
        ...
        assert_cli_output_contains(captured.out, "dry")
        assert_cli_output_contains(captured.out, "test_stage")

    def test_cli_run_single_stage_execute(self, config_file, pipeline_registry, provider_registry, capsys):
        """Test CLI run command with single stage execution."""
        test_args = ["--config", str(config_file), "run", "test_pipeline", "--stage", "test_stage"]
    ...
    def test_cli_run_phase_execution(self, config_file, pipeline_registry, provider_registry, capsys):
        """Test CLI run command with phase execution."""
        test_args = ["--config", str(config_file), "run", "multi_phase_pipeline", "--phase", "phase1"]
    ...
    def test_stage_execution_with_context_validation(self, project_root):
        ...
        context = ContextBuilder("test_pipeline").with_data("required_data", "test_value").build()
    ...
    def test_stage_execution_context_validation_failure(self, project_root):
        ...
        assert "required_data" in result.message
    ...
    def test_provider_access_in_stage(self, config_file, project_root):
        ...
        context = ContextBuilder("test_pipeline").with_providers(
    ...
    def test_phase_execution_fail_fast(self, project_root):
        ...
        class FailFastPipeline(MockPipeline):
            def __init__(self):
                super().__init__("fail_fast_pipeline", ["failure_stage", "success_stage"])
    ...
    def test_cli_argument_validation_and_pipeline_lookup(self, config_file, pipeline_registry, provider_registry, capsys):
        """Test CLI argument validation and pipeline lookup."""
        # Test invalid pipeline
        test_args = ["--config", str(config_file), "run", "nonexistent_pipeline", "--stage", "test_stage"]
    ...
    def test_dry_run_execution_plan_display(self, config_file, pipeline_registry, provider_registry, capsys):
        """Test dry run execution plan display."""
        test_args = ["--config", str(config_file), "run", "test_pipeline", "--phase", "test_phase", "--dry-run"]
    ...
    def test_cli_execution_with_invalid_stage(self, config_file, pipeline_registry, provider_registry, capsys):
        """Test CLI execution with invalid stage name."""
        test_args = ["--config", str(config_file), "run", "test_pipeline", "--stage", "invalid_stage"]
    ...
    def test_cli_execution_with_invalid_phase(self, config_file, pipeline_registry, provider_registry, capsys):
        """Test CLI execution with invalid phase name."""
        test_args = ["--config", str(config_file), "run", "test_pipeline", "--phase", "invalid_phase"]
    ...
    def test_complete_execution_workflow(self, config_file, pipeline_registry, provider_registry, project_root):
